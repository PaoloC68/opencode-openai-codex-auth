...
`;
		...
		it('should keep parsing valid events after a malformed one', async () => {
			const sseContent = `data: {"type":"response.started"
data: {"type":"response.done","response":{"id":"resp_def","output":"ok"}}
`;
			const response = new Response(sseContent);
			const headers = new Headers();
			...
			expect(body).toEqual({ id: 'resp_def', output: 'ok' });
		});
	});
});
